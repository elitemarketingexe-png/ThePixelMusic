...
package com.unshoo.pixelmusic.data.lossless.qobuz
...
object QobuzAudioProvider {
    ...
    // PixelMusic: evaluated on each call so a pool URL entered at runtime is honoured.
    private val instanceDiscoverySources: List<String>
        get() =
            PoolAccountManager.baseUrlOrNull
    ...
    fun discoverInstances(): List<String> {
        ...
        for (source in instanceDiscoverySources) {
            runCatching {
                ...
                PoolAccountManager.effectiveReadKey.takeIf { it.isNotBlank() }?.let { key ->
        ...
        for (backend in available) {
            ...
            if (download.isPreview) {
                ...
                if (backend.isToken && backend.isPoolPremium && backend.poolId != null) {
                    com.unshoo.pixelmusic.data.lossless.pool.PoolAccountManager.report("qobuz", "account", backend.poolId, "not_premium")
                }
                markInstanceFailed(backend.id, hardFailure = false)
                continue
            }
    ...
    fun searchCandidates(
        ...
        if (tokens.isEmpty()) {
            val poolTokens = runCatching {
                com.unshoo.pixelmusic.data.lossless.pool.PoolAccountManager.qobuzAccounts().map {
                    QobuzToken(
                        token = it.token,
                        appId = it.appId,
    ...
    private fun searchItemsDirect(
        ...
        client.newCall(directRequest(url.toString(), token)).execute().use { response ->
            if (response.code == 401 || response.code == 403) {
                token.poolId?.let { com.unshoo.pixelmusic.data.lossless.pool.PoolAccountManager.report("qobuz", "account", it, "dead") }
            }
            if (!response.isSuccessful) return null
            val body = response.body?.string().orEmpty()
            if (body.isBlank()) return null
