...
package com.unshoo.pixelmusic.data.lossless.pool
...
object PoolAccountManager {
    ...
    data class PoolStatusReport(
        val generatedAt: String,
        val categories: List<PoolCategoryStatus>,
    )
    ...
    // ── PixelMusic adaptation ─────────────────────────────────────────────────────────────
    // ArchiveTune bakes the pool URL + read key in at build time only. PixelMusic additionally
    // lets the user enter both in Settings → Lossless Sources; [applyRuntimeConfig] mirrors the
    // DataStore values into these fields (and the App observes the keys so edits apply live).
    // A non-blank runtime value always wins over the BuildConfig one.
    @Volatile
    private var runtimeBaseUrl: String = ""

    @Volatile
    private var runtimeApiKey: String = ""

    /** Push the user-entered pool URL / key into the manager. Blank values fall back to BuildConfig. */
    fun applyRuntimeConfig(baseUrl: String?, apiKey: String?) {
        val newUrl = baseUrl?.trim().orEmpty()
        val newKey = apiKey?.trim().orEmpty()
        val changed = newUrl != runtimeBaseUrl || newKey != runtimeApiKey
        runtimeBaseUrl = newUrl
        runtimeApiKey = newKey
        if (changed) {
            // A different deployment/key means the cached feed (and its backoff state) is stale.
            lastRefreshAt = 0L
            lastFeedFailureAt = 0L
            lastLaunchRefreshAt = 0L
        }
    }

    /** The read key currently in force: runtime override first, then BuildConfig. */
    val effectiveReadKey: String
        get() = runtimeApiKey.ifBlank { BuildConfig.SOURCE_PROVIDER_KEY.trim() }

    /** The configured base URL (before normalisation): runtime override first, then BuildConfig. */
    val configuredBaseUrl: String
        get() = runtimeBaseUrl.ifBlank { BuildConfig.SOURCE_PROVIDER_URL.trim() }

    val isEnabled: Boolean
        get() = configuredBaseUrl.isNotBlank()

    /** True when a read key is available from either source; the feed answers 401 without one. */
    val hasReadKey: Boolean
        get() = effectiveReadKey.isNotBlank()
    ...
    /** Normalised pool base URL, exposed for the discovery endpoints used by the Tidal/Qobuz providers. */
    val baseUrlOrNull: String? get() = poolBaseUrl

    private val sourcesUrl: String? get() = poolBaseUrl?.let { "$it/api/sources" }
    ...
    suspend fun refresh(
        ...
        withContext(Dispatchers.IO) {
            ...
            refreshMutex.withLock {
                ...
                val primaryUrl = sourcesUrl ?: accountsUrl
                val fallbackUrl = accountsUrl.takeIf { it != primaryUrl }
                if (primaryUrl == null) {
                    lastFeedError = null
                    Timber.tag(TAG).d("No Source Pool URL configured; nothing to refresh")
                } else {
                    val readKey = effectiveReadKey
                    poolApiKey = readKey.ifBlank { null }

                    var result = fetchAccounts(context, primaryUrl, readKey)

                    if (!result.succeeded && result.code == 404 && fallbackUrl != null) {
                        Timber.tag(TAG).d("/api/sources returned 404; falling back to /api/accounts")
                        result = fetchAccounts(context, fallbackUrl, readKey)
                    }
    ...
    /**
     * Fetch live public aggregate health metrics from GET /api/status.
     * Shows alive, serving/premium, pending, and dead counts for all services.
     */
    suspend fun fetchPoolStatus(customBaseUrl: String? = null): PoolStatusReport? = withContext(Dispatchers.IO) {
        val base = (customBaseUrl?.trim()?.takeIf { it.isNotEmpty() } ?: poolBaseUrl) ?: return@withContext null
        ...
        try {
            ...
            client.newCall(request).execute().use { response ->
                ...
                val catsArray = root.optJSONArray("categories") ?: return@withContext null
                val list = mutableListOf<PoolCategoryStatus>()
                for (i in 0 until catsArray.length()) {
                    val obj = catsArray.optJSONObject(i) ?: continue
                    list += PoolCategoryStatus(
                        service = obj.optString("service", ""),
                        kind = obj.optString("kind", ""),
                        label = obj.optString("label", ""),
                        total = obj.optInt("total", 0),
                        alive = obj.optInt("alive", 0),
                        premium = obj.optInt("premium", 0),
                        dead = obj.optInt("dead", 0),
                        pending = obj.optInt("pending", 0),
                    )
                }
                PoolStatusReport(generatedAt = genAt, categories = list)
            }
        } catch (e: Exception) {
            Timber.tag(TAG).w(e, "Failed to fetch pool status from %s", url)
            null
        }
    }
    ...
    fun report(
        ...
        reportScope.launch {
            runCatching {
                ...
                val readKey = poolApiKey?.takeIf { it.isNotBlank() } ?: effectiveReadKey
                if (readKey.isNotBlank()) {
                    builder.header("Authorization", "Bearer $readKey")
                }
